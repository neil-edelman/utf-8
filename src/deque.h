...
/** Manages a linked-list of blocks. Only the `head` can have a block-size of
 zero. */
struct t_(deque) { struct pT_(block) *head; };
...
pT_(type) *T_(new)(struct t_(deque) *);
pT_(type) *T_(append)(struct t_(deque) *, size_t);
...
static struct t_(deque) t_(deque)(void)
	{ struct t_(deque) d; d.head = 0; return d; }
...
static void t_(deque_)(struct t_(deque) *const deque) {
	if(!deque) return;
	while(deque->head) {
		struct pT_(block) *const block = deque->head;
		deque->head = block->previous;
		free(block);
	}
	*deque = t_(deque)();
}
