...
static bool binary_is_delimit(const char *const string_in_utf8,
	...
	if(byte < 0x80) { /* 1 byte? */
		internal.u8[0] = byte;
		edge = u32_upper_bound(utf8_delimit_edges,
			0, utf8_delimit_end[0], internal.u32);
		*output_next = string.unconst + 1;
		...
		internal.u8[0] = byte;
		edge = u32_upper_bound(utf8_delimit_edges,
			utf8_delimit_end[0], utf8_delimit_end[1], internal.u32);
		*output_next = string.unconst + 2;
		...
		internal.u8[0] = byte;
		edge = u32_upper_bound(utf8_delimit_edges,
			utf8_delimit_end[1], utf8_delimit_end[2], internal.u32);
		*output_next = string.unconst + 3;
		...
		internal.u8[0] = byte;
		edge = u32_upper_bound(utf8_delimit_edges,
			utf8_delimit_end[2], utf8_delimit_end[3], internal.u32);
...
static bool binary_is_not_delimit(const char *const string_in_utf8,
		const char **const output_next) {
	const union {
		const char *input;
		const uint8_t *unchar;
	...
	if(byte < 0x80) { /* 1 byte? */
		...
		edge = u32_upper_bound(utf8_not_delimit_edges,
			0, utf8_not_delimit_end[0], internal.u32);
		*output_next = string.unconst + 1;
	} else if((byte & 0xe0) == 0xc0) { /* 2 bytes continued? */
		internal.u8[1] = byte;
		byte = string.unchar[1];
